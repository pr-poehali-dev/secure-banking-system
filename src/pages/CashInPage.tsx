...
═══════════════════════════════════════════════════
Подпись вносителя: ___________________
Подпись кассира: ___________________
  ...
  a.href = url;
  a.download = `ОКУД-0402008_${data.date.replace(/\./g, '-')}.txt`;
  a.click();
  URL.revokeObjectURL(url);
}

export default function CashInPage({ clients, accounts, employee, onTransaction, onAccountCreated }: CashInPageProps) {
      ...
      <div className="p-6 flex items-center justify-center min-h-[calc(100vh-80px)]">
        <div className="w-full max-w-md animate-scale-in">
          <div className="glass-card rounded-2xl p-8 text-center">
            <div className="w-16 h-16 rounded-full bg-blue-500/15 border-2 border-blue-500/30 flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-muted-foreground text-sm font-mono mb-6">ВЗНОС НАЛИЧНЫХ · ОКУД 0402008</p>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-400/25 py-2.5 rounded-xl font-mono text-sm font-semibold transition-all"
              >
    ...
    <div className="p-6 animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl bg-blue-400/10 border border-blue-400/20 flex items-center justify-center">
          <Icon name="ArrowDownToLine" size={20} className="text-blue-400" />
        </div>
        <div>
          <h1 className="text-xl font-semibold text-foreground">Взнос наличных</h1>
          <p className="font-mono text-xs text-muted-foreground">ОКУД 0402008 · Приходный кассовый ордер</p>
        </div>
      </div>
              ...
              <button
                ...
                className={`w-full text-left p-3 rounded-lg border transition-all ${
                  selectedClient?.id === client.id
                    ? 'bg-blue-400/10 border-blue-400/30 text-foreground'
        ...
        <div className="glass-card rounded-xl p-5">
          ...
          {selectedClient ? (
            <div className="space-y-4">
              <div className="p-3 bg-blue-400/5 border border-blue-400/15 rounded-lg">
                <div className="flex items-center gap-2">
                  <Icon name="UserCheck" size={14} className="text-blue-400" />
                    ...
                    {clientAccounts.map(acc => (
                      ...
                      >
                        <div className="flex justify-between">
                          ...
                          <span className="text-blue-400 font-semibold font-mono">{formatMoney(acc.balance)}</span>
                        </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-muted border border-border rounded-lg px-3 py-2 font-mono text-sm focus:outline-none focus:border-blue-400 transition-all text-foreground placeholder:text-muted-foreground/50"
                />
                {clientAccounts.length === 0 && (
                  <button
              ...
              <div>
                <label className="block font-mono text-xs text-muted-foreground mb-1.5 uppercase tracking-wider">Сумма взноса (₽)</label>
                <input
                  value={amount}
                  ...
                  type="number"
                  min="1"
                  className="w-full bg-muted border border-border rounded-lg px-3 py-2 font-mono text-lg focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400/20 transition-all text-foreground placeholder:text-muted-foreground/50"
                />
                {selectedAccount && (
                  <p className="font-mono text-xs text-muted-foreground mt-1">
                    Текущий остаток: <span className="text-blue-400">{formatMoney(selectedAccount.balance)}</span>
                  </p>
                )}
              </div>
              ...
              <button
                ...
                className="w-full bg-blue-500 hover:bg-blue-400 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50 font-mono"
              >
                {isLoading ? (
      ...
      {showCreateAccount && selectedClient && (
        <AccountCreateModal
          ...
          returnLabel="← Вернуться к операции взноса"
        />
      )}
    </div>
  );
